...
const double example_cells = 10000; // cells of 1d
const double room_width = 4.0; // in meters
const double lb_cell_size = room_width/example_cells; // conversion factor of length to lb_length
const double lb_cell_sized_squared = pow((lb_cell_size),2);
...
const double lb_viscosity = (relaxation_time - 0.5) * cs2;
const double real_time_conversion = (lb_viscosity)*(lb_cell_sized_squared/real_viscosity); // 1 time step is equivalent to this in real time
const double velocity_conversion = (lb_cell_size/real_time_conversion); // 1 lb_velocity multiplied by this is real velocity
