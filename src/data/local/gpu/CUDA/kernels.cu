...
__constant__ double d_lattice_buoyancy;
__constant__ double d_room_temp;
__constant__ double d_max_temp;
__constant__ double d_lattice_stefan_boltzmann;
...
void initCudaLattice() {
    ...
    cudaMemcpyToSymbol(d_lattice_buoyancy, &lattice_buoyancy, sizeof(double));
    ...
    cudaMemcpyToSymbol(d_lattice_stefan_boltzmann, &lattice_stefan_boltzmann, sizeof(double));
}
...
__global__ void collisionKernel(int height, int width,
    ...
    double buoyancy = d_lattice_buoyancy * (temp - d_room_temp);
    double uyF = 0.0;
...
__global__ void computeRadiationFluxKernel(int n_viewFactors,
    ...
    double heatFlux = d_lattice_stefan_boltzmann * vf_factor[i] *
