...
int main() {
    ...
    assert(gridNew.g1[getIndex(1, 0)] == 99.0);
    
    // The packet at (0,0) moving left should have wrapped around to (2,0) in g3.
    assert(gridNew.g3[getIndex(2, 0)] == 55.0);

    // Ensure it left the original spot
    assert(gridNew.g1[getIndex(0, 0)] == 0.0);
